...
router.post("/test-redis", requireAdmin, async (req, res) => {
  try {
    ...
    // Use provided password or fallback to database/environment
    const finalRedisPassword =
      (redisPassword && redisPassword.trim()) || credentials.redis.password;

    if (!credentials.redis.endpoint) {
      return res.status(400).json({
        success: false,
        message: "Redis endpoint not configured in settings",
      });
    }
...
router.post("/test-r2", requireAdmin, async (req, res) => {
  try {
    ...
    console.log("🔧 [R2 TEST] Database credentials check:");
    console.log(
      "  - Access Key ID:",
      credentials.cloudflareR2.accessKeyId
        ? `${credentials.cloudflareR2.accessKeyId.substring(0, 8)}...`
        : "NOT SET"
    );
    console.log(
      "  - Secret Access Key:",
      credentials.cloudflareR2.secretAccessKey
        ? `${credentials.cloudflareR2.secretAccessKey.substring(0, 8)}...`
        : "NOT SET"
    );
    console.log(
      "  - Endpoint S3:",
      credentials.cloudflareR2.endpointS3 || "NOT SET"
    );
    console.log("  - Bucket:", credentials.cloudflareR2.bucket || "NOT SET");
...
// Get public environment status (no auth required)
router.get("/env-status", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: {
        hasRedisEndpoint: !!credentials.redis.endpoint,
        hasR2Credentials: !!(
          credentials.cloudflareR2.bucket &&
          credentials.cloudflareR2.token &&
          credentials.cloudflareR2.accessKeyId &&
          credentials.cloudflareR2.secretAccessKey &&
          credentials.cloudflareR2.endpointS3
        ),
        // Basic status only - no sensitive credential details
        status: {
          redis: credentials.redis.endpoint ? "configured" : "missing",
          cloudflareR2: !!(
            credentials.cloudflareR2.bucket &&
            credentials.cloudflareR2.token &&
            credentials.cloudflareR2.accessKeyId &&
    ...
    res.status(500).json({
      success: false,
      message: "Error retrieving environment status",
    });
  }
});
...
router.get("/config", requireAdmin, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: {
        defaultBucketName: credentials.cloudflareR2.bucket || "hmern",
        hasRedisEndpoint: !!credentials.redis.endpoint,
        hasR2Credentials: !!(
          credentials.cloudflareR2.bucket &&
          credentials.cloudflareR2.token &&
        ...
        credentials: {
          bucketName: credentials.cloudflareR2.bucket || "NOT CONFIGURED",
          apiToken: maskCredential(credentials.cloudflareR2.token, 2),
          accessKeyId: maskCredential(credentials.cloudflareR2.accessKeyId, 2),
          secretAccessKey: maskCredential(
            credentials.cloudflareR2.secretAccessKey,
            2
          ),
          endpointS3: maskUrl(credentials.cloudflareR2.endpointS3),
          redisEndpoint: maskUrl(credentials.redis.endpoint),
        },
      },
    });
  } catch (error) {
