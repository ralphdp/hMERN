...
const initializeRedis = async (settings) => {
  try {
    ...
    const client = redis.createClient({
      ...
      password: credentials.redis.password || undefined,
      socket: {
        connectTimeout: 5000,
        lazyConnect: true,
      },
...
const logActivity = async (level, message, metadata = {}) => {
  try {
    const { WebPerformanceLog } = require("./models");
    const log = new WebPerformanceLog({
      level,
      message,
      metadata,
      timestamp: new Date(),
    ...
    console.error("Error logging web performance activity:", error);
  }
};
    ...
    try {
      ...
      if (!settings?.general?.enabled) {
        return res.status(503).json({
          success: false,
          message: "Web Performance Optimization is currently disabled",
        });
      }

      // Check specific feature if provided
      if (featureName && settings.features && !settings.features[featureName]) {
        return res.status(503).json({
          success: false,
...
const errorHandler = (err, req, res, next) => {
  console.error("Web Performance Error:", err);

  // Log error to database
  setImmediate(() => {
    logActivity("error", `Web Performance Error: ${err.message}`, {
      stack: err.stack,
      url: req.originalUrl,
      method: req.method,
      ip: req.ip,
  ...
  res.status(500).json({
    success: false,
    message: "Internal server error in Web Performance Optimization",
